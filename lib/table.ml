...
module MakeITable (KV : KeyValue) : ITable with type key := KV.key = struct
  type key = KV.key
  ...
  let empty () : 'a table = Hashtbl.create sizeHint
  let enter (table, key, value) = Hashtbl.replace table key value

  let look (table, key) : 'a option =
    try Some (Hashtbl.find table key) with _ -> None

  exception KeyNotFound

  let look_exn table key : 'a =
    try Hashtbl.find table key with _ -> raise KeyNotFound
  ...
  let union_exn tbl1 tbl2 =
    ...
    Hashtbl.iter
      (fun k v ->
        ...
        | Some v1 ->
            if not (v1 = v) then failwith "Key exists with different value!"
  ...
  let iter f (table : 'a table) =
    let keys = ref [] in
    Hashtbl.iter (fun k _ -> keys := k :: !keys) table;
  ...
  let print (strKey : key -> string) (strVal : 'a -> string) (tbl : 'a table) :
      unit =
    print_endline ("Size: " ^ string_of_int (Hashtbl.length tbl));
    Hashtbl.iter
      (fun k v ->
        print_string "Entry: ";
        print_string (strKey k);
        print_string " ";
        print_string (strVal v);
        print_endline "")
      tbl
end

module Make (KV : KeyValue) = struct
  type key = KV.key
